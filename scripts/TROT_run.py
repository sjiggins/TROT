...
def main():
    ...
    hist_x0, hist_x1, edges_x0, edges_x1 = utils.plotting.plot_distributions(test_nominal_r[0], test_target_r[0]+7,
    ...
    hist_x0 /= np.sum(hist_x0)
    hist_x1 /= np.sum(hist_x1)
    ...
    for j in range(nq):
        for i in range(nl):
            print(q[j],l[i])
            P_tmp = TROT(q[j],M,hist_x1,hist_x0,l[i],1E-2)
    ...
    for b in range(nl):
        for a in range (nq):
            ...
            print(f'W_{q[a],l[b]} = {np.sum(P[nl*a + b]*M)}')
            
    for i in range(nq):
        ax_row = plt.Subplot(fig,outer_row_marg[i], sharey = ax)
        ax_row.plot(hist_x1, np.arange(0, np.shape(radial_positions)[0]) )#radial_positions)
